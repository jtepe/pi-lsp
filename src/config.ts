...
function parseServerConfig(value: unknown, index: number): LspServerConfig {
  ...
  if (
    ...
    commandRecord.command.trim().length === 0
  ) {
    throw new Error(
      `Server ${index} command.command must be a non-empty string`,
    );
  }
  ...
  if (
    record.treeSitterLanguage !== undefined &&
    typeof record.treeSitterLanguage !== "string"
  ) {
  ...
  if (
    ...
    record.trace !== "verbose"
  ) {
    throw new Error(
      `Server ${index} trace must be one of off, messages, verbose`,
    );
  }
  return {
    ...
    rootMarkers: isStringArray(record.rootMarkers)
      ? record.rootMarkers
      : undefined,
    command: {
...
function loadConfigFromDir(
  ...
  for (const candidate of CONFIG_CANDIDATES) {
    ...
    const defaultLimit =
      typeof parsed.defaultLimit === "number"
        ? parsed.defaultLimit
        : DEFAULT_LIMIT;
    const maxLimit =
      typeof parsed.maxLimit === "number" ? parsed.maxLimit : DEFAULT_MAX_LIMIT;
...
export function serversForWorkspace(
...
export function matchServer(
  ...
  const language =
    options.language ??
    (absoluteFilePath ? detectLanguageFromPath(absoluteFilePath) : undefined);
  ...
  const candidates = config.servers.filter((server) => {
    ...
    if (relativeFilePath && server.fileGlobs && server.fileGlobs.length > 0) {
      return server.fileGlobs.some((glob) =>
        minimatch(relativeFilePath, glob, { dot: true }),
      );
    }
    return true;
  });
