...
function flattenDocumentSymbols(
  ...
  for (const symbol of symbols) {
    ...
    if (symbol.children && symbol.children.length > 0) {
      result.push(
        ...flattenDocumentSymbols(path, uri, symbol.children, symbol.name),
      );
    }
  }
  return result;
...
export class LspClient {
  ...
  static async start(match: WorkspaceMatch): Promise<LspClient> {
    const child = spawn(
      match.server.command.command,
      match.server.command.args ?? [],
      {
        cwd: match.rootPath,
    ...
    child.once("exit", () => {
      for (const pending of state.pending.values()) {
        pending.reject(
          new Error(`LSP server ${match.server.id} exited unexpectedly`),
        );
      }
      state.pending.clear();
    });
    ...
    const timeout = new Promise<never>((_, reject) => {
      setTimeout(
        () =>
          reject(
            new Error(
              `LSP server ${match.server.id} initialization timed out after ${LspClient.INIT_TIMEOUT_MS}ms`,
            ),
          ),
  ...
  private consume(chunk: Buffer): void {
    ...
    while (true) {
      if (this.state.contentLength === null) {
        ...
        const header = this.state.buffer
      ...
      if (
        ...
        this.state.buffer.byteLength < this.state.contentLength
      ) {
        return;
      }
      const payload = this.state.buffer
  ...
  private notify(method: string, params: unknown): void {
    this.state.process.stdin?.write(
  ...
  private ensureProcessAlive(): void {
    if (this.state.process.exitCode !== null) {
      throw new Error(
        `LSP server ${this.state.match.server.id} is not running`,
      );
    }
  }
  ...
  async workspaceSymbols(query: string): Promise<SymbolInformation[]> {
    ...
    return await this.request<SymbolInformation[]>("workspace/symbol", {
      query,
    });
  }
  ...
  async documentSymbols(path: string): Promise<ResolvedSymbolLocation[]> {
    ...
    const result = await this.request<
      Array<SymbolInformation | DocumentSymbol>
  ...
  async references(
    ...
    this.ensureProcessAlive();
    await this.openDocument(path);
    const locations = await this.request<LspLocation[] | null>(
