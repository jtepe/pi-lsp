...
const codeSymbolsParameters = Type.Object({
  ...
  filePath: Type.Optional(
    Type.String({
      description: "File path used for server selection and symbol resolution",
    }),
  ),
  language: Type.Optional(
    Type.String({ description: "Language override for server selection" }),
  ),
  serverId: Type.Optional(
    Type.String({ description: "Explicit LSP server id from .pi/lsp.json" }),
  ),
  includeSource: Type.Optional(
    Type.Boolean({ description: "Include source code snippets in results" }),
  ),
  limit: Type.Optional(
...
async function attachSource(
  ...
  for (const location of locations) {
    const source = await sourceExtractor.expandSymbolSource(
      location,
      serverTreeSitterLanguage,
    );
...
async function findDocumentSymbolCandidate(
  ...
  candidate: ResolvedSymbolLocation;
  treeSitterLanguage: string | undefined;
}> {
  if (!params.filePath) {
  ...
  if (!candidate) {
    throw new Error(
      `No document symbol matching "${params.name}" found in ${params.filePath}`,
    );
  }
...
export function createCodeSymbolsTool(
  ...
  typeof codeSymbolsParameters,
  { serverId: string; rootPath: string; count: number }
> {
  return {
    name: "code_symbols",
    label: "Code Symbols",
    description:
      "Search symbols and resolve definitions or references through configured LSP servers.",
    promptSnippet:
      "Search code symbols, definitions, and references using project-configured LSP servers.",
    promptGuidelines: [
    ...
    async execute(
      ...
      if (params.action === "search") {
        ...
        const text =
          withSource.length === 0
            ...
            : withSource
                .map((location) => formatResultBlock(location))
                .join("\n");
        return {
      ...
      const { candidate, treeSitterLanguage } =
        await findDocumentSymbolCandidate(config, manager, ctx.cwd, params);
      ...
      const resolved =
        params.action === "definitions"
          ? await client.definition(
              resolve(ctx.cwd, params.filePath!),
              candidate,
            )
