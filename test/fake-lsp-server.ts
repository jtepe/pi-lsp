...
const fixtures = {
  typescript: {
    ...
    definition: (workspaceRoot: string) => [
  ...
  python: {
    ...
    definition: (workspaceRoot: string) => [
      createLocation(`${workspaceRoot}/pkg/tool.py`, 0, 4, 15),
    ],
    references: (workspaceRoot: string) => [
      createLocation(`${workspaceRoot}/pkg/tool.py`, 0, 4, 15),
    ],
...
function respond(message: JsonRpcMessage): void {
  ...
  if (message.method === "workspace/symbol") {
    ...
    send({
      jsonrpc: "2.0",
      id: message.id,
      result: [fixture.workspaceSymbol(workspaceRoot)],
    });
    return;
  }
  ...
  if (message.method === "textDocument/definition") {
    ...
    send({
      jsonrpc: "2.0",
      id: message.id,
      result: fixture.definition(workspaceRoot),
    });
    return;
  }
  ...
  if (message.method === "textDocument/references") {
    ...
    send({
      jsonrpc: "2.0",
      id: message.id,
      result: fixture.references(workspaceRoot),
    });
    return;
  }
